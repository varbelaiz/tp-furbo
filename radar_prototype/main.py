...
# --- TEAM AND BALL COLOURS ---
COLOR_TEAM_A = sv.Color.from_hex('#FF1493') # Pink
COLOR_TEAM_B = sv.Color.from_hex('#00BFFF') # Blue
COLOR_BALL = sv.Color.from_hex('#FFD700')   # Gold
...
def process_video(video_path: str, detections_path: str, calibration_path: str, actions_path: str, output_path: str):
    ...
    try:
        ...
        print("Error: one of the JSON files is missing")
        return
    ...
    print("Setting up the pitch rendering...")
        ...
        if detections and calibration and "homography_inverse" in calibration:
            try:
                ...
                if player_points_2D is not None:
                    ...
                    # 7. Composite the radar onto the overlay
                    y0 = RADAR_MARGIN_PX
                    y1 = y0 + RADAR_H_PX
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Overlay calibration, detections and actions onto a video.")
    parser.add_argument("--video", type=str, required=True, help="Path to the input video (e.g. video_input.mp4).")
    parser.add_argument("--detections", type=str, required=True, help="Path to the detections JSON (e.g. detections.json)")
    parser.add_argument("--calibration", type=str, required=True, help="Path to the calibration JSON (e.g. calibration.json)")
    parser.add_argument("--actions", type=str, required=True, help="Path to the actions JSON (e.g. actions.json)") 
    parser.add_argument("--output", type=str, required=True, help="Path to write the output video to (e.g. video_output.mp4)")
