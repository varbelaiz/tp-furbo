...
This directory holds the camera calibration data used to build the keypoint
dataset. Everything in it is generated; nothing is tracked in git.
...
`unified_output/` follows the layout Ultralytics expects:
...
3. Generates `unified_output/` (29 keypoints).

It finishes with:

```
PIPELINE COMPLETE
   29 KP: data/calibration/unified_output
```
