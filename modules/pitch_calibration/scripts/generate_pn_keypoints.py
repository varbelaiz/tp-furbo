...
HEATMAP_SIZE = (480, 270) # W, H (half resolution)
SCALE_X = HEATMAP_SIZE[0] / IMG_SIZE[0]
SCALE_Y = HEATMAP_SIZE[1] / IMG_SIZE[1]
SIGMA = 1

# --- Network 1 configuration (keypoints) ---
NUM_NET1_CHANNELS = 58 

# --- Network 2 configuration (lines) ---
NET2_LINE_NAMES = [
...
def parse_soccernet_json(json_path):
    try:
        ...
        print(f"\nWarning: could not read the JSON {json_path}: {e}")
...
def process_single_annotation(json_path):
    ...
    # --- Network 1 (keypoints), following Falaleev ---
    try:
    ...
    # --- Network 2 (lines), following PnLCalib ---
...
def main():
    ...
    for data_dir in DATASET_DIRS:
        ...
        if not os.path.exists(data_dir):
            print(f"Warning: input directory not found: {data_dir}. Skipping.")
            continue
