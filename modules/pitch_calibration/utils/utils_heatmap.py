...
def draw_label_map(img, pt, sigma, type='Gaussian'):
    ...
    # Make sure the point is expressed as integers
    pt = (int(pt[0]), int(pt[1]))

    # Bounds check
    if pt[0] < 0 or pt[0] >= img_width or pt[1] < 0 or pt[1] >= img_height:
        return img  # Leave the image untouched when the point is out of bounds
    ...
    # Region of the image the kernel will be pasted into
    g_x_min = max(0, -ul[0])
    g_x_max = min(br[0], img_width) - ul[0]
    g_y_min = max(0, -ul[1])
    g_y_max = min(br[1], img_height) - ul[1]

    # Corresponding region of the heatmap
    ...
    if g_x_max > g_x_min and g_y_max > g_y_min:
        # Paste the kernel into the heatmap with np.maximum
