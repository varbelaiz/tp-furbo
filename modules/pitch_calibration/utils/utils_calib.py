    ...
    def get_cam_params(self, mode='full', use_ransac=0, refine=False, refine_w_lines=False):
        ...
        if len(obj_pts) < 6:
            try:
                ...
                print(f"    -> WARNING: cv2.calibrateCamera (fewer than 6 points) failed. Error: {e}")
                print("    -> Most likely collinear points. Skipping this frame.")
                return None, None  # None signals the failure to the caller
            ...
            if not np.isnan(np.min(mtx)):
                ...
                try:
                    ...
                    print(f"    -> WARNING: cv2.calibrateCamera (more than 6 points) failed. Error: {e}")
                    print("    -> Most likely collinear points. Skipping this frame.")
                    return None, None  # None signals the failure to the caller
            else:
                print("    -> WARNING: initCameraMatrix2D returned NaN. Skipping this frame.")
                ret = False
                return None, None  # Return None here as well
        ...
        for mode in ['full', 'ground_plane', 'main']:
            for use_ransac in [0, 5, 10, 15, 25, 50]:
                ...
                if cam_params is None or ret is None:
                    # get_cam_params failed, so abort and return None
                    return None
