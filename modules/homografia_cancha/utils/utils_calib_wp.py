...
keypoint_world_coords_2D = [[x - 52.5, 34. - y] for x, y in keypoint_world_coords_2D]
keypoint_aux_world_coords_2D = [[x - 52.5, 34. - y] for x, y in keypoint_aux_world_coords_2D]
line_world_coords_3D = [[[x1 - 52.5, 34. - y1, -z1], [x2 - 52.5, 34. - y2, -z2]] for [[x1, y1, z1], [x2, y2, z2]] in line_world_coords_3D]
...
class FramebyFrameCalib:
    ...
    def update(self, calibration, kp_dict, lines_dict):
        ...
        self.principal_point = np.array([self.intrinsics[0, 2], self.intrinsics[1, 2]])
    ...
    def reproj_err(self, obj_pts, img_pts):
        if self.intrinsics is not None:
            ...
            for i in range(len(obj_pts)):
                obj_point = np.array([*obj_pts[i], 1.0])
                cam_point = rotation @ It @ obj_point
                x, y, z = cam_point[0], cam_point[1], cam_point[2]
                x_norm = x / z
                y_norm = y / z
                ...
                err_point = img_pts[i] - np.array([u, v])
    ...
    def projection_from_cam(self):
        It = np.eye(4)[:-1]
        It[:, -1] = -self.position
        P = self.intrinsics @ (self.rotation @ It)
        return P
    ...
    def line_optimizer(self, vector, img_pts, obj_pts):
        P = vector_to_mtx(vector, self.intrinsics)
    ...
    def get_cam_params(self, mode='full', use_ransac=0, refine=False, refine_w_lines=False):
        flags = cv2.CALIB_FIX_ASPECT_RATIO | cv2.CALIB_USE_INTRINSIC_GUESS | cv2.CALIB_FIX_FOCAL_LENGTH | \
                cv2.CALIB_FIX_PRINCIPAL_POINT | cv2.CALIB_FIX_S1_S2_S3_S4 | cv2.CALIB_FIX_TAUX_TAUY | \
        ...
        if ret:
            ...
            if refine:
                if not np.isnan(rep_err):
                    ...
                    rvecs, tvecs = cv2.solvePnPRefineLM(obj_pts, img_pts, self.intrinsics, self.distortion,
                                                        rvec, tvec,
            ...
            cam_params = {
                ...
                "tilt_degrees": tilt,
                "roll_degrees": roll,
                "x_focal_length": self.intrinsics[0, 0],
                "y_focal_length": self.intrinsics[1, 1],
                ...
                "radial_distortion": [self.distortion[0], self.distortion[1], self.distortion[4], 0., 0., 0.],
                "tangential_distortion": [self.distortion[2], self.distortion[3]],
